...
def create_relative_postion_old(image1, image2, c, scale):
    '''
...
def create_relative_postion(image1, image2, c, scale):
    '''
    takes an image and normalises it into a 3d object facing -z. y is hieght and x is width. 
    then uses 8 point algorithm to find image2's relative postion to image1.
    Args:
        image1: 2d image points, tensor
        image2: 2d image points, tensor
        ...
        scale: 1d tensor, used to scale the epipole vector which defines distance between camera origins  
    Returns:
        image_mesh1: image1 as normlaised mesh with no rotation or location transform. 
        image_mesh2: image2 with transforms placing it relative to image1
        offset1: image2 offset from image1
    ...
    mat_rot = torch.tensor((
                        (-0.05946767330169678, 0.5062960982322693, -0.8603068590164185),
                        (-0.9978998303413391, -0.007980247028172016, 0.06428220123052597),
                        (0.025680365040898323, 0.8623228073120117, 0.5057073831558228)))

    offset_inverted = offset1 * -1
    image_mesh2 = transform_mesh_tensor(image_mesh2, mat_rot)
    image_mesh2 = offset_verts(image_mesh2, offset_inverted)

    return image_mesh1, image_mesh2, offset1, mat_rot
...
def train():
    ...
    images = np.load(f'{dir1}//multi_cam_imgs.npy')
    images = torch.from_numpy(images)

    scale_list = torch.ones(((images.shape[0])) ,requires_grad=True).float()
    c = torch.tensor((-.01))
    lr = 0.1
    criterion = torch.nn.MSELoss()
    optimizer1 = torch.optim.Adam([scale_list], lr=lr)
    epochs = 200
    image_size = 1080
    ...
    for epoch in range(epochs):
        ...
        offset_list = []
        image_list = []
        for image, scale in zip(images[1:], scale_list[1:]):
            image1, image2, offset = create_relative_postion(images[0], image, c, scale)
    ...
    for i, image in enumerate(image_list):
        create_object(image, f'image{i}')
    create_object(image1, 'image_1')
